...
interface BookingBarProps {
  onFilter?: (rooms: Room[]) => void;
}

const BookingBar = ({ onFilter }: BookingBarProps) => {
  ...
  const handleSearch = () => {
    ...
    if (availableRooms.length > 0) {
      ...
      toast.error("No rooms available for these dates. Showing all rooms.");
      onFilter?.(allRooms);
    }
  };
  ...
  return (
    <motion.div 
      ...
      className="relative z-20 -mt-12 px-4 md:px-6 w-full max-w-5xl mx-auto"
    >
      <div className="bg-background/80 backdrop-blur-2xl border border-border rounded-[2rem] md:rounded-[2.5rem] p-2 md:p-4 shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex flex-col md:flex-row items-center gap-2 md:gap-4">
